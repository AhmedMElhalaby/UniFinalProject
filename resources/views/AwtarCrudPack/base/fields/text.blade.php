<div class="@if(isset($Field['col'])) {{$Field['col']}} @else col-md-12 @endif" id="{{$Field['name']}}_div">
    <div class="form-group label-floating">
        ...
        <input type="text" id="{{$Field['id'] ?? $Field['name']}}" name="{{$Field['name']}}" @if($Field['is_required']) required @endif class="form-control {{ $errors->has($Field['name']) ? ' is-invalid' : '' }}" value="{{$value}}">
    </div>
